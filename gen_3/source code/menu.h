...
int get_started(SDL_Renderer*renderer,bool&sound, SDL_Texture*bk,TTF_Font*F)
{
    ...
    SDL_Color impressed={100,100,100},white ={255,255,255};
    ...
    s=IMG_Load("about.jpg");
    SDL_SetColorKey( s, SDL_TRUE, SDL_MapRGB( s->format, 0, 0xFF, 0xFF ) );
    PIC[10]=SDL_CreateTextureFromSurface(renderer,s);
   // PIC[10]=IMG_LoadTexture(renderer,"about.jpg");
    ...
    while(!quit)
    {
      ...
      bool start=false,sod=false,about=false,q=false;
             ...
             if(e.type==SDL_MOUSEBUTTONDOWN)
        {
            ...
            if((x>=WIDTH-WM)&&(x<=WIDTH))
            {
                ...
                else if(y>=HEIGHT/2-HM){sod=true;if(sound) {sound=false;Mix_PauseMusic();} else {sound=true;Mix_ResumeMusic();};}
                else return 1;
      ...
      if(!start){ SDL_RenderCopy(renderer,PIC[0],NULL,&R[0]);} else SDL_RenderCopy(renderer,PIC[5],NULL,&R[0]);
      if(!sod)
      {
          if(sound==true){SDL_RenderCopy(renderer,PIC[1],NULL,&R[1]);} else SDL_RenderCopy(renderer,PIC[2],NULL,&R[1]);
      } else {if(sound==true){SDL_RenderCopy(renderer,PIC[6],NULL,&R[1]);} else SDL_RenderCopy(renderer,PIC[7],NULL,&R[1]);};
      if(!about){ SDL_RenderCopy(renderer,PIC[3],NULL,&R[2]);} else{ SDL_RenderCopy(renderer,PIC[8],NULL,&R[2]);SDL_RenderCopy(renderer,PIC[10],NULL,&aa);};
      if(!q){ SDL_RenderCopy(renderer,PIC[4],NULL,&R[3]);} else SDL_RenderCopy(renderer,PIC[9],NULL,&R[3]);
