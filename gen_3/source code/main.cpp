...
int main(int argc, char*argv[])
{
   ...
   Mix_Music *Sound=NULL;
    ...
    if(Sound==NULL) std::cout<<"error load Sound"<<std::endl;
 ...
 bool lost=false;
 do{
if(!getInput(renderer,b,barrier,BB,WALL,bk,F,score)){continue_=false;break;};
 ...
 line l[11];
  ...
  if (renderRaw(renderer,b,barrier,l,11,bk,BB,WALL,sound,F,score)==false) {if(QuitOrNot(renderer,barrier,F,bk,WALL,score)) continue_=false;lost=true;}
  else if(b.P.y+2<now_pos){score++; update(renderer,b,barrier,BB,bk,WALL,F,score);};} while(!lost);

};
  if(i==4) continue_=false;
};
