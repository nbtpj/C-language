...
//this function will return true if the ball still be on the screen or false if the ball got out of the screen
{
 ...
 pic =IMG_LoadTexture(renderer,"background.jpg");
  if(pic==NULL) cout<<SDL_GetError()<<endl;*/
    while(!EXIT)/*&&(quit==false)*/
      {
        ...
        pos={b.P.x-S/2,b.P.y-S/2,S,S};
          ...
          {
              if(a.type==SDL_QUIT/*||a.type==SDL_KEYDOWN*/) qu=true;
          };
          if(qu) break;
 ...
 //         for(int i=0;i<sz;i++) line::renderLine(no[i],renderer);
...
//          R->x=b.P.x-15;R->y=b.P.y-15;R->h=30;R->h=30;
       //  SDL_RenderDrawPoint(renderer,b.P.x,b.P.y);
         //    drawBall(b,renderer);
...
void update(SDL_Renderer*renderer,ball&b, SDL_Rect barrier[],SDL_Texture*BB,SDL_Texture*bk,SDL_Texture*WALL,TTF_Font*F,long&score)
{
    ...
    SDL_Rect new_rect={rand()%(WIDTH-W),rand()%(HEIGHT*3/4-3*S)+S,W*2,H};
 ...
 //   SDL_Delay(20);
    ...
    {
        cout<<"no"<<endl;
        cout<<"ball: "<<b.P.x<<";"<<b.P.y<<endl;
        cout<<"rec[0].y="<<barrier[0].y<<endl;
        cout<<"rec[1].y="<<barrier[1].y<<endl;
    }
